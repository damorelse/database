...
int main(int argc, char *argv[])
{
   ...
   for (numTest = 0; numTest < (int)(sizeof(RecSize)/4); numTest++) {
      ...
      cout << "Opening filescan...reading strings of length ";
      cout.flush();
      int strLength = recSize;    //GINA
      if (strLength > 255)        //GINA
         strLength = 255;         //GINA
      cout << strLength << "..."; //GINA
      result = scan.OpenScan(fh, STRING, strLength, 0, NO_OP, NULL);  //GINA
      if (result) goto ERROR;
      cout << "Success!\n";
      cout.flush();
